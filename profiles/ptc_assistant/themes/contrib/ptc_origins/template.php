 ...
 */
function ptc_origins_theme($existing, $type, $theme, $path) {
  return array(
    ...
    'form_content' => array(
      'render element' => 'form',
      'path' => drupal_get_path('theme', 'ptc_origins') . '/templates/form',
 ...
 */
function ptc_origins_preprocess_html(&$variables, $hook) {
  global $theme_key;

  $site_name = variable_get('site_name', 'PTC Assistant');
 ...
 */
function ptc_origins_menu_link($variables) {
  $output = '';
  ...
  ptc_origins_menu_link_class($element);
  $sub_menu = '';
 ...
 */
function ptc_origins_menu_link_class(&$element)  {
 ...
 */
function ptc_origins_preprocess_page(&$variables, $hook) {
 ...
 */
function ptc_origins_preprocess_node(&$variables, $hook) {
 ...
 */
function ptc_origins_preprocess_two_33_66(&$variables, $hook) {
  $menu = menu_get_item();
...
function ptc_origins_preprocess_three_25_50_25(&$variables, $hook) {
  $menu = menu_get_item();
 ...
 */
function ptc_origins_preprocess_panels_pane(&$variables, $hook) {
  $pane = $variables['pane'];
 ...
 */
function ptc_origins_preprocess_views_view(&$variables, $hook) {
  $view = $variables['view'];
 ...
 */
function ptc_origins_preprocess_form(&$variables, $hook) {
 ...
 * need to perform some processing on attributes to make it work in a template.
 */
function ptc_origins_process_form(&$variables, $hook) {
 ...
 */
function ptc_origins_preprocess_form_content(&$variables, $hook) {
  // Bootstrap the with some of Drupal's default variables.
  template_preprocess($variables, $hook);
 ...
 */
function ptc_origins_process_form_content(&$variables, $hook) {
  // Crunch down attribute arrays.
  template_process($variables, $hook);
}
 ...
 */
function ptc_origins_preprocess_views_view_unformatted(&$variables, $hook) {
 ...
 */
function ptc_origins_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function ptc_origins_css_alter(&$css) {
 ...
 * will not render.
 */
function ptc_origins_links($variables) {
 ...
 * characters.
 */
function _ptc_origins_format_address(&$address) {
 ...
 */
function ptc_origins_field__addressfield($variables) {
  $output = '';
  ...
  foreach($variables['items'] as &$address) {
    ...
    if ($address['#address']['administrative_area']) {
      _ptc_origins_format_address($address);
    }
    else {
 ...
 */
function ptc_origins_preprocess_views_view_field(&$variables, $hook) {
  ...
  if (isset($variables['theme_hook_suggestion']) && $variables['theme_hook_suggestion'] == 'views_view_field__field_address') {
    foreach ($variables['row']->field_field_address as $key => &$address) {
      if (!$address['raw']['administrative_area']) {
        ...
        _ptc_origins_format_address($address['rendered']);
      }
    }
